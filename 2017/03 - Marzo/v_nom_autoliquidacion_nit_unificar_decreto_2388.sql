...
SELECT DISTINCT orden='1',tipo=(case when cpto.clase_concepto=9 or cpto.clase_concepto=11 then 'VAC' end ), 
       ...
       dias_total = sum(isnull(liq.dias,0)) FROM v_nom_autoliquidacion_unificar as v JOIN
...
WHERE cpto.clase_concepto IN (9,11) and liq.turno<>50 and (year(liq.fecha_ini) = v.ano and month(fecha_ini)=v.mes and year(liq.fecha_fin) = v.ano and month(fecha_fin)=v.mes) GROUP BY (case when cpto.clase_concepto=9 or cpto.clase_concepto=11 then 'VAC' end ),
      liq.ano,year(liq.fecha),month(liq.fecha),liq.nit,liq.contrato,liq.numero,liq.fecha_ini,liq.fecha_fin
HAVING (( case when day(liq.fecha_ini) = 31 or day(liq.fecha_fin) = 31 then ((sum(isnull(liq.dias,0))) - 1) else  (sum(isnull(liq.dias,0))) end) > 0)  and (isnull(sum(liq.valor),0)) > 0 UNION
/****** licencias remuneradas y no remuneradas ******/ SELECT DISTINCT orden='2',tipo=( case when cpto.clase_concepto=4 then 'LR' else
     ...
     y_liquidacion                  as liq ON liq.nit=v.nit and liq.contrato=v.contrato and year(liq.fecha)=v.ano and month(liq.fecha)=v.mes JOIN 
	 y_conceptos_nom                as cpto on cpto.concepto=liq.concepto
WHERE cpto.clase_concepto IN (4,24,27) and liq.turno<>50 and (year(liq.fecha_ini) = v.ano and month(fecha_ini)=v.mes and year(liq.fecha_fin) = v.ano and month(fecha_fin)=v.mes) GROUP BY (case when cpto.clase_concepto=4 then 'LR' else
          case when cpto.clase_concepto=24 or cpto.clase_concepto = 27 then 'SLN'
         end end ),
		 liq.ano,liq.mes,liq.nit,v.contrato,liq.concepto,liq.numero,liq.fecha_ini,liq.fecha_fin
HAVING (( case when day(liq.fecha_ini) = 31 or day(liq.fecha_fin) = 31 then ((sum(isnull(liq.dias,0))) - 1) else  (sum(isnull(liq.dias,0))) end) >= 1) UNION
/****** incapacidades menos 180 dias y pagadas en el mismo mes ******/ SELECT DISTINCT orden='3',tipo=( case when cpto.clase_concepto=12 OR cpto.clase_concepto=33 then 'INC' end ), 
...
WHERE cpto.clase_concepto IN (12,33) and liq.turno<>50 and (year(liq.fecha_ini) = v.ano and month(fecha_ini)=v.mes and year(liq.fecha_fin) = v.ano and month(fecha_fin)=v.mes) GROUP BY ( case when cpto.clase_concepto=12 OR cpto.clase_concepto=33 then 'INC' end ),
       v.ano,v.mes,v.nit,v.contrato,liq.numero,liq.fecha_ini,liq.fecha_fin
HAVING (( case when day(liq.fecha_ini) = 31 or day(liq.fecha_fin) = 31 then ((sum(isnull(liq.dias,0))) - 1) else  (sum(isnull(liq.dias,0))) end) >= 1) UNION
/****** incapacidades igual mas 180 dias cuando no son liquidadas ******/ SELECT DISTINCT orden='4',tipo='INC', 
...
GROUP BY  v.ano,v.mes,liq.nit,liq.contrato UNION
/****** Ausencias extemporaneas  ******/ SELECT DISTINCT orden='5',tipo='SAL' , v.ano,v.mes,nomina=max(liq.nomina),v.nit,contrato=max(v.contrato),
       ...
       dias_total =  sum(isnull(v.dias_cotizar,0)) FROM v_nom_autoliquidacion_unificar as v JOIN 
...
WHERE (cpto.clase_concepto in (4,9,11,12,33) and liq.ano = v.ano and liq.mes=v.mes and 
	  (year(liq.fecha_ini) <> liq.ano or month(liq.fecha_ini) <> liq.mes or year(liq.fecha_fin) <> liq.ano or month(liq.fecha_fin) <> liq.mes )) GROUP BY v.ano,v.mes,v.nit HAVING ((( case when sum(isnull(liq.dias,0)) > 30 then 30 else sum(isnull(liq.dias,0))  end ) >= 1)  or ((sum(v.dias_cotizar)) > 0)) and
...
SELECT DISTINCT orden='6',tipo='SAL' , v.ano,v.mes,nomina=max(liq.nomina),v.nit,contrato=max(v.contrato),
     ...
     y_liquidacion                  as liq ON liq.nit=v.nit and liq.contrato=v.contrato  and liq.ano=v.ano and liq.mes=v.mes JOIN 
     y_conceptos_nom                as cpto ON cpto.concepto=liq.concepto
WHERE (cpto.clase_concepto IN (1,2,16) and liq.turno<>50) GROUP BY v.ano,v.mes,v.nit HAVING ((( case when sum(isnull(liq.dias,0)) > 30 then 30 else sum(isnull(liq.dias,0))  end ) >= 1)  or ((sum(v.dias_cotizar)) > 0)) and
         (sum(isnull(liq.valor,0)) > 0 ) UNION
...
SELECT DISTINCT orden='7',tipo='SAL' , v.ano,v.mes,nomina=max(liq.nomina),v.nit,contrato=max(v.contrato),
       ...
       valor = 0,
       dias_total =  sum(isnull(v.dias_cotizar,0)) FROM v_nom_autoliquidacion_unificar as v LEFT JOIN 
     y_personal_contratos           as liq ON liq.nit=v.nit and liq.contrato=v.contrato  and liq.turno<>50 
WHERE v.tipo='retiros' and liq.fecha_retiro <> liq.fecha_final_salud GROUP BY v.ano,v.mes,v.nit HAVING (sum(v.dias_cotizar) > 0 ) 
