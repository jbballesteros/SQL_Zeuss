...
/****** Object:  Job [Proceso_Bonificaciones]    Script Date: 04/06/2017 16:24:39 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 04/06/2017 16:24:39 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Proceso_Bonificaciones', 
		...
		@notify_email_operator_name=N'Operador BD', 
		@notify_page_operator_name=N'Operador BD', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FVIMPUESTOS]    Script Date: 04/06/2017 16:24:39 ******/
...
SELECT     D.tipo AS TIPO, D.numero AS NUMERO, SUM(CASE WHEN R.subgrupo2 IN (13, 20, 21) THEN CONVERT(real, (((DL.cantidad) * DL.valor_unitario) * (1 - isnull(DL.porcentaje_descuento, 0) 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DEVOLUCIONES]    Script Date: 04/06/2017 16:24:39 ******/
...
SELECT     tipo,numero,tipo_link, numero_link, codigo, can_real
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [TERCEROS]    Script Date: 04/06/2017 16:24:39 ******/
...
LEFT JOIN terceros_23 T23 ON (TC.concepto_23=T23.concepto_23)', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FACTURA COMPLETA]    Script Date: 04/06/2017 16:24:39 ******/
...
WHERE     (D.sw IN (1)) AND (D.anulado = 0) AND (R.subgrupo2 IN (''2'', ''3'', ''4'', ''12'', ''13''))', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS_ACUMULADO]    Script Date: 04/06/2017 16:24:39 ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS]    Script Date: 04/06/2017 16:24:39 ******/
...
GROUP BY TIPO, NUMERO, valor_total, VALORCM, valor_aplicado, VENCIMIENTO, VENCIMIENTOREAL', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [BONIFICACION]    Script Date: 04/06/2017 16:24:39 ******/
...
SELECT     YEAR(FV.fecha) AS AÑO, MONTH(FV.fecha) AS MES, DAY(FV.fecha) AS DIA, FV.tipo, FV.numero, FV.fecha, FV.VENCIMIENTOREAL AS vencimiento, FV.VENCIMIENTO AS VENCIMIENTOF, 
                      ...
                      WHEN FV.NIT_REAL = 890943038 AND FV.fecha BETWEEN ''01/11/2014'' AND ''01/11/2014'' THEN (CASE WHEN DIAS BETWEEN ''-8'' AND 
