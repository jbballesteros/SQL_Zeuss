...
SELECT 14, @TIPO, ROW_NUMBER() OVER (ORDER BY D.NUMERO)+@MAXIMO, 899999068, D.fecha, D.condicion, D.vencimiento,0, D.iva, D.retencion, D.retencion_causada, D.retencion_iva, D.retencion_ica, D.descuento_pie, D.fletes, D.iva_fletes, 
                      D.costo, D.vendedor, 0, D.anulado, '*', D.numero, 'SUBIR COSTO TARIFA DE TRANSPORTE DE BIOCOMBUSTIBLE: ' + D.tipo + CAST(D.NUMERO AS VARCHAR(10)), D.usuario, D.pc,DATEADD(MINUTE,1, D.fecha_hora), D.retencion2, D.retencion3, D.bodega, D.impoconsumo, D.descuento2, 
...
WHERE YEAR(D.FECHA)=@AÑO AND MONTH(D.FECHA)=@MES   AND D.sw IN (11,12) AND concepto=7 AND DL.adicional LIKE '%BODO:1000%' AND D.bodega=@BODEGA AND R.subgrupo2= 8
