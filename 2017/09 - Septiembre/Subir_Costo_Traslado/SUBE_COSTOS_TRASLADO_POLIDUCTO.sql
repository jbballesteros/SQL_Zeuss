...
DECLARE @TIPO AS VARCHAR(10)='SCP'
SELECT @MAXIMO=MAX(NUMERO) FROM DOCUMENTOS WHERE TIPO=@TIPO

SELECT 14, @TIPO, ROW_NUMBER() OVER (ORDER BY D.NUMERO)+@MAXIMO, 900531210, D.fecha, D.condicion, D.vencimiento,0, D.iva, D.retencion, D.retencion_causada, D.retencion_iva, D.retencion_ica, D.descuento_pie, D.fletes, D.iva_fletes, 
                      D.costo, D.vendedor, 0, D.anulado, '100', D.numero, 'SUBIR COSTO TARIFA DE POLIDUCTO: ' + D.tipo + CAST(D.NUMERO AS VARCHAR(10)), D.usuario, D.pc,DATEADD(MINUTE,1, D.fecha_hora), D.retencion2, D.retencion3, D.bodega, D.impoconsumo, D.descuento2, 
