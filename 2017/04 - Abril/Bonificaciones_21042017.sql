...
/****** Object:  Job [Proceso_Bonificaciones]    Script Date: 04/21/2017 11:13:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 04/21/2017 11:13:50 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Proceso_Bonificaciones', 
		...
		@notify_email_operator_name=N'Operador DB', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FVIMPUESTOS]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     D.tipo AS TIPO, D.numero AS NUMERO, SUM(CASE WHEN R.subgrupo2 IN (13, 20, 21) THEN CONVERT(real, (((DL.cantidad) * DL.valor_unitario) * (1 - isnull(DL.porcentaje_descuento, 0) 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DEVOLUCIONES]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     tipo,numero,tipo_link, numero_link, codigo, can_real
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [TERCEROS]    Script Date: 04/21/2017 11:13:50 ******/
...
CASE WHEN ISNUMERIC(T21.descripcion)=1 THEN CAST(T21.descripcion AS REAL) ELSE 0 END AS BON_MARGEN_3, 
CASE WHEN ISNUMERIC(T23.descripcion)=1 THEN CAST(T23.descripcion AS REAL) ELSE 0 END AS BON_PESOS_3,  
...
LEFT JOIN terceros_21 T21 ON (TC.concepto_21=T21.concepto_21)
LEFT JOIN terceros_23 T23 ON (TC.concepto_23=T23.concepto_23)', 
		@database_name=N'zeuss', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FACTURA COMPLETA]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     D.tipo, D.numero, D.fecha, D.valor_total, CASE WHEN YEAR(D .fecha) < 2013 THEN (D .valor_total - FVIMPUESTOS.SOBRETASA) 
                    ...
                    (CASE WHEN DL.fec <= ''31/05/2015'' THEN 
                     (CASE WHEN R.subgrupo2 IN (2, 3) THEN (T.BON_PESOS_3)+ (T.BON_MARGEN_3 * 3.264) ELSE (T.BON_PESOS_3)+ (T.BON_MARGEN_3 * 4.43) END) 
                     ELSE  (T.BON_PESOS_3)+ (T.BON_MARGEN_3 * (CB.margen / 100))  END) * (DL.cantidad + ISNULL(DV.can_real, 0)) 
                    AS BONIFICACION3,                      
...
FROM         dbo.documentos AS D INNER JOIN
                ...
                LEFT OUTER JOIN
                      dbo.categorias_bonificacion AS CB ON ( R.subgrupo2 = CB.codigo AND D.fecha BETWEEN CB.desde AND CB.hasta) LEFT JOIN
                      bon_devolucion_tbl DV ON ( DL.tipo = DV.tipo_link AND DL.numero = DV.numero_link AND DL.codigo = DV.codigo)                   
                      

WHERE     (D.sw IN (1)) AND (D.anulado = 0) AND (R.subgrupo2 IN (''2'', ''3'', ''4'', ''12'', ''13'')) AND D.fecha>=''01/01/2017''', 
		@database_name=N'zeuss', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS_ACUMULADO]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     DC.tipo_aplica AS TIPO, DC.numero_aplica AS NUMERO, CASE WHEN DC.tipo = ''NC'' THEN AB.fecha ELSE (CASE WHEN ISDATE(DCC.documento)=1  THEN (CASE WHEN (YEAR(DCC.documento)>2050 OR YEAR(DCC.documento)<2000) THEN AB.fecha ELSE  DCC.documento END ) ELSE AB.fecha END) END AS FECHAAB, DC.valor AS VALORAB,
                                               ...
                                               FROM          dbo.documentos_cruce AS DC INNER JOIN
                                                                      ...
                                                                      LEFT JOIN documentos_che DCC ON (AB.tipo=DCC.tipo AND AB.numero=DCC.numero)
                                               WHERE      (DC.sw = 1)
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     TIPO, NUMERO, MIN(FECHA_PAGO) AS FECHA_PAGO, DATEDIFF(D, VENCIMIENTOREAL, MIN(FECHA_PAGO)) AS DIASREAL, DATEDIFF(D, VENCIMIENTO, MIN(FECHA_PAGO)) AS DIAS, 
                      CASE WHEN VALOR_TOTAL = VALOR_APLICADO THEN (CASE WHEN SUM(VALORCOMPENSADO) >= VALOR_TOTAL AND SUM(ABONOAV) > 0 AND SUM(ABONODV) > 0 THEN 0 ELSE 0 END) 
                      ELSE CASE WHEN SUM(VALORCOMPENSADO) >= VALORCM AND SUM(ABONOAV) > 0 AND SUM(ABONODV) > 0 THEN 0 ELSE 0 END END AS COMPENSA
...
GROUP BY TIPO, NUMERO, valor_total, VALORCM, valor_aplicado, VENCIMIENTO, VENCIMIENTOREAL', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [BONIFICACION]    Script Date: 04/21/2017 11:13:50 ******/
...
SELECT     YEAR(FV.fecha) AS AÑO, MONTH(FV.fecha) AS MES, DAY(FV.fecha) AS DIA, FV.tipo, FV.numero, FV.fecha, FV.VENCIMIENTOREAL AS vencimiento, FV.VENCIMIENTO AS VENCIMIENTOF, 
                      ...
                      WHEN FV.NIT_REAL = 890943038  THEN 
                      (CASE 
                      WHEN ZP.DIAS < - 8 THEN FV.BONIFICACION 
                      WHEN ZP.DIAS BETWEEN ''-8'' AND ''0'' THEN FV.BONIFICACION2 
                      WHEN ZP.DIAS BETWEEN 1 AND 7 THEN FV.BONIFICACION3 ELSE 0 END) 
                      ...
                      900247772) AND ZP.DIAS <= 1 THEN FV.BONIFICACION 
                      
                      WHEN FV.CONCEPTO_2 = ''40'' AND ZP.DIAS <= 2 THEN FV.BONIFICACION 
                      
                      WHEN FV.NIT_REAL = 22384461 THEN 
                      (CASE WHEN DIAS <= 0 THEN FV.BONIFICACION2 
                      WHEN ZP.DIAS > 0 THEN FV.BONIFICACION ELSE 0 END) 
...
WHERE     (FV.subgrupo2 IN (2, 3, 4)) AND YEAR(FV.fecha)>=2017

UNION ALL

SELECT *
FROM v_bonificaciones_lubricantes) AS P', 
		@database_name=N'zeuss', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Diario', 
		...
		@schedule_uid=N'1a5e6913-f312-4b0b-89da-0b7b2298e8d2'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
