...
WHERE PF.str_error='Pago Exitoso' AND se_interfazo IS NULL AND PF.Empresa='RyRLubricantes'
...
WHILE @@FETCH_STATUS = 0  
	BEGIN
		...
		FROM v_PSE_facturas
		WHERE registro=@ID AND Empresa='RyRLubricantes' AND tipo IS NOT NULL
		...
		BEGIN TRAN @TSQL
			...
			SELECT 'RC',@SIG,1,'11100551',1000,0 ,CAST(fecha_transaccion AS DATE),valor_pagado,T.nombres,valor_pagado
			FROM v_PSE_pagosfacturas_tbl PF INNER JOIN terceros T ON (CAST(id_cliente AS DECIMAL)=T.nit)
			WHERE PF.id_pago=@ID
			UNION
			SELECT 'RC',@SIG,2,'130505',1000,0 ,CAST(fecha_transaccion AS DATE),valor_pagado*-1,T.nombres,valor_pagado*-1
			FROM v_PSE_pagosfacturas_tbl PF INNER JOIN terceros T ON (CAST(id_cliente AS DECIMAL)=T.nit)
			WHERE PF.id_pago=@ID
		...
		SELECT 2,@ID,'RC',@SIG,@RST,@ERROR,GETDATE(),0
