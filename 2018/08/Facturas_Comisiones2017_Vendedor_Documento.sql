...
SELECT   fcd.id,  fcd.tipo, fcd.numero, fcd.TipoCruce, fcd.NumeroCruce, fcd.dcto2, fcd.fecha, fcd.fecha_real as fecha_rango,cast(fcd.fechacruce as date) as fecha_real,  
 ...
 rt1.descripcion ='COMPETITIVE' then 1 when rt1.descripcion ='PREMIUM' then 1.25 when rt1.descripcion ='FLAGSHIP' then 1.50 ELSE 1 end as porc_comision_categoria,  
                      fcd.Dias, d.nit, ter.nombres, 
					  d.vendedor, 
					  case  when d.vendedor between 100 and 199 then 'ANTIOQUIA'   
                      when d.vendedor between 200 and 299 then 'EJE CAFETERO' 
					  when d.vendedor between 300 and 399 then 'COSTA' when   
                      d.vendedor between 400 and 499 then 'BOGOTA' else 'OTROS' end as sede, vend.nombres as nomvendedor,  
...
FROM         dbo.FacturasPorCategoDias2013 AS fcd INNER JOIN  
                      ...
                      inner join terceros as ter on d.nit=ter.nit left join terceros as vend on d.vendedor=vend.nit  
