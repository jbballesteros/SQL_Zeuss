...
--SELECT 14, 'SCP', ROW_NUMBER() OVER (ORDER BY D.NUMERO)+@NUMERO, 899999068, D.fecha, D.condicion, D.vencimiento,0, D.iva, D.retencion, D.retencion_causada, D.retencion_iva, D.retencion_ica, D.descuento_pie, D.fletes, D.iva_fletes, 
--                      D.costo, D.vendedor, 0, D.anulado, '*', D.numero, 'SUBE COSTO TRANSPORTE TARIFA MARCACION', D.usuario, D.pc,DATEADD(MINUTE,1, D.fecha_hora), D.retencion2, D.retencion3, D.bodega, D.impoconsumo, D.descuento2, 
...
--                      D.descuento_1, D.descuento_2, D.descuento_3, D.abono, D.fecha_consignacion, 10, D.concepto_Retencion, D.porc_RteFuente, D.porc_RteIva, D.porc_RteIvaSimpl, D.porc_RteIca, 
--                      D.porc_RteA, D.porc_RteB, D.ajuste, D.bodega_ot, D.numero_ot, D.porc_RteCree, D.retencion_cree, D.codigo_retencion_cree, D.cree_causado, D.numincapacidad, D.idincapacidad, D.nota1, 
--                      D.nota2, D.placa, D.conductor, D.sicom, D.cupo_frontera, D.ObligacionFinanciera, D.provision, D.Base_dcto_RC, D.remolque, D.Iva_Costo
...
INNER JOIN referencias_SubeBajaCosto RS ON (R.id=RS.id_referencia AND RS.id_clase IN (10))
...
WHERE YEAR(D.fecha)=@AÑO AND MONTH(D.fecha)=@MES AND D.sw IN (12) AND D.concepto=7 AND DL.adicional LIKE '%BODO:1000%'  AND DD.clase_tra=10
ORDER BY DL.numero
