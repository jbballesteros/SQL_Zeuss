...
SELECT  DD.SW, DD.tipo, DD.numero, DL.codigo, ROW_NUMBER () OVER (PARTITION BY DL.NUMERO ORDER BY DL.TIPO,DL.NUMERO), DL.fec, 900531210, 1, DL.porcentaje_iva, ROUND(DL.cantidad*RS.valor,2), DL.porcentaje_descuento, ROUND(DL.cantidad*RS.valor,2), 
...
WHERE D.fecha BETWEEN '01/06/2016' AND '30/06/2016' AND D.sw IN (11,12) AND D.concepto=7 AND DL.adicional LIKE '%BODO:1000%' AND D.bodega=2
AND DD.modelo='100'
ORDER BY DL.numero
