...
(CV.cuenta BETWEEN '510595' AND '51059508') OR 
(CV.cuenta BETWEEN '520503' AND '520539') OR 
(CV.cuenta IN ('52054503','520548','52059503','52059508'))) and ano=2014 AND mes=12
GROUP BY CV.nit
...
from cuentas_val CV  
where (
(CV.cuenta BETWEEN '511010' AND '511095') OR 
(CV.cuenta BETWEEN '52109501' AND '52109595')) and ano=2014 AND mes=12
GROUP BY CV.nit
HAVING SUM(CV.saldo_inicial+CV.debito-CV.credito )<>0
-----FORMATO 1001 CONCEPTO 5002
UNION
...
where (
(CV.cuenta BETWEEN '510572' AND '510578') or (CV.cuenta BETWEEN '520572' AND '520578')   ) and ano=2014 AND mes=12
GROUP BY CV.nit
HAVING SUM(CV.saldo_inicial+CV.debito-CV.credito )<>0
-----FORMATO 1001 CCONCEPTO 5010
UNION
...
where (
  ...
  (CV.cuenta BETWEEN '5215' AND '521575')  ) and ano=2014 AND mes=12
GROUP BY CV.nit
HAVING SUM(CV.saldo_inicial+CV.debito-CV.credito )<>0
...
from cuentas_val CV  
where (
(CV.cuenta IN ('510584','51059509','520551','520560','520563','520584','52059502','530505','530515','530595','530596') OR 
...
(CV.cuenta LIKE '5230%') OR
(CV.cuenta LIKE '5240%') OR
(CV.cuenta LIKE '5250%') OR
(CV.cuenta LIKE '5255%') OR
(CV.cuenta LIKE '5295%') OR
(CV.cuenta LIKE '5299%') OR
(CV.cuenta LIKE '5310%') OR
(CV.cuenta LIKE '5315%') OR
(CV.cuenta LIKE '5395%') OR
(CV.cuenta LIKE '530535%') OR
(CV.cuenta LIKE '530535%') ))  and ano=2014 AND mes=12 
GROUP BY CV.nit
HAVING SUM(CV.saldo_inicial+CV.debito-CV.credito )<>0
...
from cuentas_val CV  
where (
(CV.cuenta LIKE '5165%' ) ) and ano=2014 AND mes=12
GROUP BY CV.nit
HAVING SUM(CV.saldo_inicial+CV.debito-CV.credito )<>0
-----FORMATO 1001 CONCEPTO 5002
) 
