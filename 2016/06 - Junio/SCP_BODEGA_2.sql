...
SELECT 14, 'SCP', ROW_NUMBER() OVER (ORDER BY D.NUMERO)+(SELECT MAX(NUMERO) FROM DOCUMENTOS WHERE TIPO='SCP'), D.nit, D.fecha, D.condicion, D.vencimiento,0, D.iva, D.retencion, D.retencion_causada, D.retencion_iva, D.retencion_ica, D.descuento_pie, D.fletes, D.iva_fletes, 
                      D.costo, D.vendedor, 0, D.anulado, '*', D.numero, D.notas, D.usuario, D.pc,DATEADD(MINUTE,1, D.fecha_hora), D.retencion2, D.retencion3, D.bodega, D.impoconsumo, D.descuento2, 
...
WHERE D.fecha BETWEEN '01/05/2016' AND '31/05/2016' AND D.sw IN (11,12) AND concepto=7 AND DL.adicional LIKE '%BODO:1000%' AND D.bodega=2
...
SELECT  DD.SW, DD.tipo, DD.numero, DL.codigo, ROW_NUMBER () OVER (PARTITION BY DL.NUMERO ORDER BY DL.TIPO,DL.NUMERO), DL.fec, DL.nit, 1, DL.porcentaje_iva, ROUND(DL.cantidad*RS.valor,2), DL.porcentaje_descuento, ROUND(DL.cantidad*RS.valor,2), 
...
INNER JOIN referencias_SubeBajaCosto RS ON (R.id=RS.id_referencia AND RS.id_clase IN (2,10,11))
...
WHERE D.fecha BETWEEN '01/05/2016' AND '31/05/2016' AND D.sw IN (11,12) AND D.concepto=7 AND DL.adicional LIKE '%BODO:1000%' AND D.bodega=2
ORDER BY DL.numero
