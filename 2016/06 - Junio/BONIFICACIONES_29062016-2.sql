...
/****** Object:  Job [Proceso_Bonificaciones]    Script Date: 06/29/2016 14:38:18 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 06/29/2016 14:38:18 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Proceso_Bonificaciones', 
		...
		@owner_login_name=N'ZEUSSPETROLEUM\sistemas', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FVIMPUESTOS]    Script Date: 06/29/2016 14:38:18 ******/
...
SELECT     D.tipo AS TIPO, D.numero AS NUMERO, SUM(CASE WHEN R.subgrupo2 IN (13, 20, 21) THEN CONVERT(real, (((DL.cantidad) * DL.valor_unitario) * (1 - isnull(DL.porcentaje_descuento, 0) 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DEVOLUCIONES]    Script Date: 06/29/2016 14:38:18 ******/
...
SELECT     tipo,numero,tipo_link, numero_link, codigo, can_real
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [TERCEROS]    Script Date: 06/29/2016 14:38:18 ******/
...
LEFT  JOIN terceros AS TV ON T.vendedor = TV.nit', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [FACTURA COMPLETA]    Script Date: 06/29/2016 14:38:18 ******/
...
WHERE     (D.sw IN (1)) AND (D.anulado = 0) AND (R.subgrupo2 IN (''2'', ''3'', ''4'', ''12'', ''13''))', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS_ACUMULADO]    Script Date: 06/29/2016 14:38:18 ******/
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [PAGOS]    Script Date: 06/29/2016 14:38:18 ******/
...
GROUP BY TIPO, NUMERO, valor_total, VALORCM, valor_aplicado, VENCIMIENTO, VENCIMIENTOREAL', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [BONIFICACION]    Script Date: 06/29/2016 14:38:18 ******/
