...
WHERE year(fecha_docs)=@AÑO AND MONTH(fecha_docs)=@MES AND DT.sw=11 AND conversion=0
...
WHERE year(DT.fecha_docs)=@AÑO AND MONTH(DT.fecha_docs)=@MES AND DT.sw_doc=12 AND DT.conversion=0) AS P ON (DL.tipo=P.tipo AND DL.numero=P.numero AND DL.codigo=P.codigo AND DL.seq=P.id_lin)
...
FROM documentos_traslados DT INNER JOIN documentos_traslados DT2 ON (DT.id=DT2.id AND DT.id_lin=DT2.id_lin AND DT2.sw_doc=11)
WHERE year(DT.fecha_docs)=@AÑO AND MONTH(DT.fecha_docs)=@MES AND DT.sw_doc=12 AND DT.conversion=0
